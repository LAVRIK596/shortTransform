...
- [Команды сборки и поддержки проекта](#команды-сборки-и-поддержки-проекта)
...
# Команды сборки и поддержки проекта
- Способ сборки проекта описан в файлах `CMakeLists.txt`, `.vscode/launch.json`, `.vscode/tasks.json`. Если вы используете `vscode`, `vscodium` или что-либо другое, что умеет работать с vscode командами, то достаточно открыть проект в нем.
- В редакторе будут настроеные запуски `Debug` и `Release` сборки.
- В редакторе так же есть дополнительные команды:
  - `Delete temporary directories` - удалить папки **documentation** и **build** вместе с её содержимым.
  - `Generate documentation` - сгенерировать doxygen документацию (html и rtf).
